class Solution {
	...
	string findOrder(vector<string> &words) {
		...
		for (const auto & word : words) {
			for (char ch : word) if (inDegrees[ch - 'a'] == -1) {inDegrees[ch - 'a'] = 0; charCount++; }
		}
		...
		for (int i = 0; i < wordCount - 1; i++) {
			...
			if (j == len && j < len1)
				return "";
			if (j < len)
				{adj[first[j] - 'a'].push_back(second[j] - 'a'); inDegrees[second[j] - 'a']++; }
		}
		...
		return answer.size() == charCount ? answer : "";
	}
};
